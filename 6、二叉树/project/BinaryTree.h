...
// 操作结果：按树状形式显示以r为根的二叉树，level为层次数，可设根结点的层次数为1
{
	if(r != NULL)	{	// 空树不显式，只显式非空树
		DisplayBTWithTreeShape<ElemType>(r->leftChild, level + 1);//显示右子树
		cout << endl;					//显示新行	
		for(int i = 0; i < level - 1; i++)
			cout << "  ";				//确保在第level列显示结点
		cout << r->data;				//显示结点
		DisplayBTWithTreeShape<ElemType>(r->rightChild, level + 1);//显示左子树
	}
}
...
// 操作结果：返回以r为根的二叉树的叶子结点个数
{
	if (r == NULL)
		return 0;// 空二叉树结点个数为0
	else
		if (LeafCount(r->leftChild) == NULL && LeafCount(r->rightChild) == NULL)
			return 1;
		else
...
/*template <class ElemType>
int BinaryTree<ElemType>::Note(int m,int n) const
{
	BinTreeNode<ElemType>* newRoot = new BinTreeNode<ElemType>(r);
	newRoot->leftChild = Revolute(r->rightChild);
	newRoot->rightChild = Revolute(r->leftChild);
	return newRoot;
}*/
