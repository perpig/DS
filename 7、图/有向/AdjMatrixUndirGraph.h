...
public:
	...
	// 最短路径
	int shortestPathLength(ElemType &from, ElemType &to) const;
	double ClosenessCentrality(ElemType &d) const; // 紧密中心度 归一化
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::InsertArc(int v1, int v2, int value)
// 操作结果：插入依附顶点v1和v2的边
{
	if (v1 < 0 || v1 >= vexNum)
		throw Error("v1不合法!"); // 抛出异常
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::CountOutDegree(ElemType &d) const
{
	int outdegree = 0;
	int v = GetOrder(d); // 获取index
	if (v == -1)
		return -1; // 没有值d的元素
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::CountInDegree(ElemType &d) const
{
	int indegree = 0;
	int v = GetOrder(d); // 获取index
	if (v == -1)
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::CountDegree(ElemType &d) const
{
	int degree = CountInDegree(d) + CountOutDegree(d);
	if (degree == -2)
		return -1; // 没有值d的元素
	return degree;
}
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::NodeInStrength(ElemType &d) const
{
	int instrength = 0;
	int v = GetOrder(d);
	if (v == -1)
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::NodeOutStrength(ElemType &d) const
{
	int outstrength = 0;
	int v = GetOrder(d);
	if (v == -1)
	...
	for (int j = 0; j < vexNum; j++)
		if (arcs[v][j] != 0)
			outstrength += arcs[v][j];
	return outstrength;
}

template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::NodeStrength(ElemType &d) const
{
	int degree = NodeInStrength(d) + NodeOutStrength(d);
	if (degree == -2)
		return -1; // 没有值d的元素
	return degree;
}
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::shortestPathLength(ElemType &from, ElemType &to) const
{
	// vector<int> last(vexNum, -1);	   // 上一个结点 用来记录路径 待补充
...
template <class ElemType>
double AdjMatrixUndirGraph<ElemType>::ClosenessCentrality(ElemType &d) const
{
	int u = GetOrder(d);
	if (u == -1)
		return -1;	 // 没找到顶点
	int dis_sum = 0; // 最短路径之和
	// 计算最短路径之和
	for (int i = 0; i < vexNum; i++)
	{
		if (i != u) // 不是自己的时候
			dis_sum += shortestPathLength(vertexes[u], vertexes[i]);
	}
	double cc, c; // 紧密中心度
	cc = (double)1 / dis_sum;
	c = (double)(vexNum - 1) / cc; // 归一化后
...
template <class ElemType>
int AdjMatrixUndirGraph<ElemType>::Count2hop(ElemType &d) const
// 2-hop
{
	vector<bool> flag(vexNum, false); // 标记
	int u = GetOrder(d);
